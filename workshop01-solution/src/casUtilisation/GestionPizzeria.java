...
public class GestionPizzeria {
    ...
    /**
     * Retirer de la commande en cours du client la pizza en quantité indiquée
     *
     * @param client   le client qui a une commande en cours à qui on supprime la
     *                 pizza; ce client appartient à la liste des clients
     * @param pizza    la pizza à retirer de la commande en cours du client ; la
     *                 pizza appartient à la carte des pizzas
     * @param quantite le nombre de fois qu'on retire la pizza de la commande en
     *                 cours du client. Si quantite est plus grand que le nombre de pizza
     *                 dans la commande, on retire simplement toutes les pizzas existantes.
     * @throws NoCommandeEnCoursException si le client n'a pas de commande en cours
     * @throws IllegalArgumentException   si la quantité est négative ou nulle
     */
    public void retirerDeLaCommande(Client client, Pizza pizza, int quantite) throws NoCommandeEnCoursException {
        Util.checkPositive(quantite);
        Util.checkObject(pizza);
        obtenirCommandeEnCours(client);
        client.getCommandeEnCours().retirer(pizza, quantite);
    }
